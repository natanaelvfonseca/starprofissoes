...
test("DEV, CEO, CVO e MARKETING podem trocar a unidade ativa", () => {
  for (const role of ["DEV", "CEO", "CVO", "MARKETING"] satisfies Array<UserRole>) {
    assert.equal(canSwitchActiveUnit(role), true);
  }
});

test("demais perfis não podem trocar a unidade ativa", () => {
  for (const role of ["DIRETOR", "GERENTE", "CONSULTOR"] satisfies Array<UserRole>) {
