...
function BI() {
  ...
  const [attendanceId, setAttendanceId] = React.useState("all");
  const [attendanceOptions, setAttendanceOptions] = React.useState<
  ...
  React.useEffect(() => {
    ...
    setScopeValue((current) => {
      ...
      return current === "all" || session.units.some((unit) => unit.id === current)
        ? current
        : "all";
    });
  ...
  React.useEffect(() => {
    if (attendanceId === "all" && data?.attendances) {
      setAttendanceOptions(data.attendances.map(({ id, name, status }) => ({ id, name, status })));
    }
  }, [attendanceId, data?.attendances]);
  ...
  return (
    <div className="space-y-6">
          ...
          session ? (
            <div className="flex flex-wrap gap-2">
              <Select value={attendanceId} onValueChange={setAttendanceId}>
                <SelectTrigger className="w-[300px] max-w-full bg-card">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-8">
        ...
        <StatCard
          label="1º contato"
          value={metricValue(isLoading, `${metrics.averageFirstContactHours.toFixed(1)}h`)}
          icon={Clock3}
          accent="warning"
        />
        <StatCard
        ...
        <ChartCard title="Evolução comercial">
          {isLoading ? (
            ...
            <GrowthEmptyPanel
              ...
              description="A série será preenchida com a entrada de novos leads."
            />
          )}
        </ChartCard>
        ...
        <ChartCard title="Funil por etapa">
          {isLoading ? (
                ...
                <BarChart data={funnelData} layout="vertical" margin={{ left: 6 }}>
                  ...
                  <Bar dataKey="leads" name="Leads" radius={[0, 5, 5, 0]}>
                    {funnelData.map((item) => (
                      <Cell
                        key={item.stage}
                        fill={item.stage === "Matriculado" ? "#377DFE" : "#F4B728"}
                      />
                    ))}
                  </Bar>
                </BarChart>
            ...
            <GrowthEmptyPanel
              icon={Activity}
              title="Funil vazio"
              description="Não há leads no período selecionado."
            />
          )}
        </ChartCard>
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard title="Performance por turma">
          {isLoading ? (
            <GrowthLoading />
            ...
            <div className="space-y-4">
              {data.attendances.map((attendance) => (
                <GrowthDataBar
                  key={attendance.id}
                  label={`${attendance.name}${attendance.status === "inactive" ? " · Inativa" : ""}`}
                  value={attendance.leads}
                  max={Math.max(...data.attendances.map((item) => item.leads), 1)}
                  detail={`${attendance.enrollments} matrículas · ${formatPercent(attendance.conversionRate)} · ${formatCurrency(attendance.confirmedRevenue)} confirmados`}
                  accent={attendance.enrollments ? "success" : "primary"}
                />
              ))}
            </div>
          ) : (
            <GrowthEmptyPanel
              icon={GraduationCap}
              title="Sem turmas no período"
              description="As turmas permanecem separadas por curso, cidade e data."
            />
          )}
        </ChartCard>
        ...
        <ChartCard title="Performance por curso">
          {isLoading ? (
            ...
            <div className="space-y-4">
              {data.courses.map((course) => (
                <GrowthDataBar
                  ...
                  accent={course.enrollments ? "success" : "primary"}
                />
              ))}
            </div>
          ) : (
            <GrowthEmptyPanel
              icon={GraduationCap}
              title="Sem cursos no período"
              description="Os cursos aparecerão conforme os leads forem roteados."
            />
          )}
        </ChartCard>
        ...
        <ChartCard title="Conversão por cidade">
          {isLoading ? (
            ...
            <GrowthEmptyPanel
              icon={Target}
              title="Sem cidades no período"
              description="A análise geográfica depende da cidade registrada no lead."
            />
          )}
        </ChartCard>
      </div>
      <ChartCard
        title={
          data?.scope.mode === "individual" ? "Minha produtividade" : "Performance dos consultores"
        }
      >
        {isLoading ? (
          <GrowthLoading />
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[720px] space-y-2">
              ...
              {data.consultants.map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-[minmax(180px,1fr)_repeat(5,100px)] gap-3 rounded-md bg-muted/25 px-3 py-3 text-sm"
                >
          ...
          <GrowthEmptyPanel
            icon={Users}
            title="Sem produtividade calculada"
            description="Os consultores aparecerão quando receberem leads."
          />
        )}
      </ChartCard>
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-7">
        ...
        <StatCard
          label="Leads Meta"
          value={metricValue(isLoading, metrics.metaLeads)}
          icon={RadioTower}
          accent="primary"
        />
        <StatCard
          label="Campanhas"
          value={metricValue(isLoading, metrics.campaignCount)}
          icon={Megaphone}
          accent="gold"
        />
        <StatCard
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(330px,.75fr)]">
        <ChartCard title="Aquisição ao longo do tempo">
          {isLoading ? (
            ...
            <GrowthEmptyPanel
              icon={Signal}
              title="Sem aquisição no período"
              description="A curva será preenchida conforme os leads entrarem."
            />
          )}
        </ChartCard>
        ...
        <ChartCard title="Participação por origem">
          {isLoading ? (
            ...
            <GrowthEmptyPanel
              icon={Megaphone}
              title="Sem origens identificadas"
              description="Cadastre ou mapeie os canais de aquisição."
            />
          )}
        </ChartCard>
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard title="Campanhas Meta por volume">
          {isLoading ? (
            ...
            <GrowthEmptyPanel
              icon={RadioTower}
              title="Sem campanhas Meta"
              description="As campanhas aparecerão quando eventos Meta estiverem ligados aos leads."
            />
          )}
        </ChartCard>
        ...
        <ChartCard title="Eficiência das campanhas">
          {isLoading ? (
            ...
            <GrowthEmptyPanel
              icon={GraduationCap}
              title="Sem conversão por campanha"
              description="A conversão será calculada quando os leads avançarem para matrícula."
            />
          )}
        </ChartCard>
      </div>
      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard title="Origens e qualidade">
          {isLoading ? (
            ...
            <div className="space-y-4">
              {data.sources.map((source) => (
                <GrowthDataBar
                  ...
                  accent={source.enrollments ? "gold" : "primary"}
                />
              ))}
            </div>
          ) : (
            <GrowthEmptyPanel
              icon={Megaphone}
              title="Sem origens"
              description="Os canais aparecerão conforme forem gravados nos leads."
            />
          )}
        </ChartCard>
        ...
        <ChartCard title="Praças com maior demanda">
          {isLoading ? (
            ...
            <GrowthEmptyPanel
              icon={Target}
              title="Sem praças identificadas"
              description="A análise regional depende da cidade roteada para o lead."
            />
          )}
        </ChartCard>
      </div>
