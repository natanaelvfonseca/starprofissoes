...
import { createFinancialPromise } from "@/lib/server/financial";
import { financialError, financialUnitFromBody } from "@/lib/server/financial-auth";

export const Route = createFileRoute("/api/financeiro/promises")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          return Response.json(
            { promise: await createFinancialPromise(unit.id, session.user.id, body ?? {}) },
            { status: 201 },
          );
        } catch (error) {
