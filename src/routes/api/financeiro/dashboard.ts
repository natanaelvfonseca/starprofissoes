...
import { getFinancialDashboard } from "@/lib/server/financial";
import { financialUnitFromRequest } from "@/lib/server/financial-auth";

export const Route = createFileRoute("/api/financeiro/dashboard")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          return Response.json(
            { dashboard: await getFinancialDashboard(unit.id, new URL(request.url).searchParams) },
            { headers: { "Cache-Control": "no-store" } },
          );
        } catch (error) {
          const invalid = error instanceof FinancialFilterError;
          return Response.json(
            { error: invalid ? error.message : "Falha ao consultar o dashboard financeiro." },
