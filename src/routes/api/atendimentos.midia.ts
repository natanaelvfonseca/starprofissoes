  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const consultantOwnMedia = Boolean(
          session?.user.role === "CONSULTOR" && params.get("consultantId") === session.user.id,
        );
        if (!session || (!requireAttendanceAccess(session) && !consultantOwnMedia)) {
        ...
        try {
          ...
          if (!media)
            return Response.json({ ok: false, error: "Mídia indisponível." }, { status: 404 });
          ...
          return Response.json(
            { ok: false, error: "Não foi possível carregar a mídia." },
            { status: 502 },
          );
        }
      },
