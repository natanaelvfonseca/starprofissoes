...
function Treinamentos() {
  ...
  return (
    <div className="space-y-6 overflow-x-hidden" onContextMenu={(event) => event.preventDefault()}>
      <section className="grid overflow-hidden rounded-[30px] border border-[#16006C]/10 bg-white shadow-[0_30px_80px_-54px_rgba(22,0,108,0.8)] lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="relative p-6 md:p-8 lg:p-10">
          <div className="absolute left-0 top-10 h-20 w-1 rounded-r-full bg-[#F4B728]" />
          <Badge className="border-[#16006C]/10 bg-[#16006C]/8 text-[#16006C]">
            <BookOpenCheck className="mr-1 h-3.5 w-3.5" /> Academia Star
          </Badge>
          <h1 className="mt-6 max-w-4xl text-3xl font-black tracking-tight text-[#07154C] md:text-5xl">
            Conhecimento que acompanha o ritmo da operação.
          </h1>
          <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground md:text-base">
            {canViewLeadership
              ? "Plataforma, vendas, escola e liderança reunidas em uma jornada prática para o time Star."
              : "Plataforma, vendas e escola reunidas em uma jornada prática para o time Star."}
          </p>
          <div className="mt-7 flex flex-wrap items-center gap-2">
            <Badge variant="secondary" className="bg-[#F4B728]/15 text-[#8A6100]">
              {activeUnitName}
            </Badge>
            <Badge variant="secondary">{summary.totalLessons} aulas disponíveis</Badge>
            <Button
              variant="ghost"
              size="sm"
              className="text-[#16006C]"
        ...
        <aside className="relative flex flex-col justify-between overflow-hidden bg-[#16006C] p-7 text-white">
          <div className="pointer-events-none absolute -right-20 -top-24 h-64 w-64 rounded-full border border-white/10" />
          <div className="relative">
            <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#F4B728]">
              Seu avanço
            </div>
            <div className="mt-3 text-6xl font-black">{summary.progressPercent}%</div>
            ...
            {visibleTrails.map((trail) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "group flex min-w-0 items-center gap-3 rounded-2xl border border-[#16006C]/10 bg-white p-3 text-left shadow-card transition duration-300 hover:-translate-y-0.5 hover:border-[#16006C]/30 hover:shadow-elegant md:justify-between",
                    active && "border-[#16006C] bg-[#16006C] text-white shadow-elegant",
                  )}
                >
