import { ensureRuntimeSchema } from "@/lib/server/db";

let financialSchemaPromise: Promise<void> | null = null;
...
create table if not exists app_financial_integrations (
  ...
  base_url text not null default 'https://app.caezescola.com.br/api/', token_encrypted text not null,
  active boolean not null default true, sync_past_days integer not null default 730 check (sync_past_days between 1 and 3650),
  sync_future_days integer not null default 365 check (sync_future_days between 1 and 3650),
  last_sync_at timestamptz, last_successful_sync_at timestamptz, last_error text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique (unit_id, provider)
);
create table if not exists app_financial_students (
  id uuid primary key default gen_random_uuid(), unit_id uuid not null references app_units(id) on delete restrict,
  external_student_id text not null, full_name text not null, cpf text, phone text, phone2 text, email text,
  raw_payload jsonb not null default '{}'::jsonb, last_synced_at timestamptz not null default now(),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), unique (unit_id, external_student_id)
);
create table if not exists app_financial_enrollments (
  id uuid primary key default gen_random_uuid(), unit_id uuid not null references app_units(id) on delete restrict,
  student_id uuid not null references app_financial_students(id) on delete cascade, external_enrollment_id text not null,
  external_class_id text, class_name text, external_course_id text, course_name text, enrollment_date date, start_date date, end_date date,
  financial_lookup_status text not null default 'NOT_FOUND' check (financial_lookup_status in ('FOUND','NOT_FOUND','NO_DOCUMENT','ERROR')),
  last_financial_lookup_at timestamptz, raw_payload jsonb not null default '{}'::jsonb,
...
create table if not exists app_financial_sync_runs (
  ...
  started_at timestamptz, finished_at timestamptz, classes_processed integer not null default 0,
  students_processed integer not null default 0, installments_found integer not null default 0,
...
create table if not exists app_financial_installments (
  ...
  enrollment_id uuid references app_financial_enrollments(id) on delete set null, external_title_id text not null, due_date date not null,
  original_amount numeric(14,2) not null default 0, penalty_amount numeric(14,2) not null default 0,
  interest_amount numeric(14,2) not null default 0, total_amount numeric(14,2) not null default 0,
  days_overdue integer not null default 0, status text not null check (status in ('upcoming','due_today','overdue','not_returned')),
  boleto_url text, course_suspended boolean not null default false, restriction_type text,
  responsible_external_id text, responsible_name text, responsible_document text, responsible_phone text, responsible_email text,
  first_seen_at timestamptz not null default now(), last_seen_at timestamptz not null default now(), not_returned_at timestamptz,
...
create table if not exists app_financial_collection_actions (
  ...
  notes text, performed_by uuid references app_users(id) on delete set null,
...
create table if not exists app_financial_promises (
  id uuid primary key default gen_random_uuid(), unit_id uuid not null references app_units(id) on delete restrict,
  student_id uuid not null references app_financial_students(id) on delete cascade,
  installment_id uuid references app_financial_installments(id) on delete set null, promised_date date not null,
  ...
  notes text, created_by uuid references app_users(id) on delete set null,
